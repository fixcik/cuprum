...
- **2026-06-03 — авторазмещение (nesting) в окне «Добавить дизайн на панель».** Между телом окна и
  ...
  **Дизайн | На панели**: «На панели» = `PanelLayoutPreview` — мини-бланк (медный inset-контур +
  ...
  `fitLine` (сетка C×R / «Войдёт N из M» жёлтым при переполнении). Числовые мм-поля — `UnitField`
  (пока показывают «mm» без конвертации в империал — общий с `PanelEditor` пробел, см. ROADMAP).
