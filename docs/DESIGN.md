...
- **2026-06-04 — схематичное превью «Дизайн» в окне + общий хук `usePreviewData`.** Режим «Дизайн»
  ...
  **без паяльной маски** (схематичный вид: медь/шелк/контур на голой подложке). Машинерия загрузки
  (SVG-слои/сверловка/3D-меш/метрики + деривация находок/маркеров/слоёв) вынесена из
  `DesignInspector` в переиспользуемый хук `hooks/usePreviewData.ts`; инспектор и окно используют
  один хук (инспектор — с правкой типов слоёв и flush'ем артефактов, окно — read-only без них).
  «Без маски» = опция `excludeMask`: отбрасывает `topMask`/`bottomMask` из `layers` (2D) и из
  `visibleKeys` (3D, клиентский фильтр меша). Окно не пишет в проект и не зовёт артефакт-flush
  (главное окно — единственный писатель). Внизу превью — компактная строка имя + `VerdictBadge`.
