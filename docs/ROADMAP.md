...
- [ ] Panel editor UI: холст в Panel space, drag BoardInstance, инструмент «ToolingHole»
  ...
  - **Добавление дизайна на панель** — отдельное OS-окно (Tauri `WebviewWindow`): выбор
    ...
    Ведём фазами:
    - [ ] Фаза 1 — окно + мост состояния (интенты) + палитра инструментов на холсте +
          добавление одной платы «плотно в угол» + минимальный read-only рендер instance'ов `[in progress]`
