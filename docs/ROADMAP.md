...
- **DFM: двусторонний дизайн на односторонней панели.** Если у дизайна есть медь на
  обеих сторонах (`Bottom`-слой непустой), а stackup панели — односторонний
  (`double_sided=false`), это физически непечатаемо. Выдавать `Finding`/ошибку при
  размещении (логика — `feasibility.ts`, факт «двусторонний» — из набора слоёв дизайна,
  `double_sided` — из манифеста панели). Чисто фронтовое поверх уже считаемых метрик.
