...
- **Экран «Операции»:** редизайн в две колонки (производственные шаги + история прогонов,
  PR #701); live-баннер активной сверловки в левой колонке — пульсирующий статус, счётчик
  отверстий, кнопка «Стоп», прогресс-бар (✅ 2026-06-17, PR #703, эпик #697); строка
  параметров операции в карточке шага (drill: отверстия/свёрла/оценка, expose:
  сторона/выдержка) из последнего прогона (✅ 2026-06-17, PR #704, #700).

- **Привязка рабочего нуля тремя методами** (эпик #705): точки центровки в модели
  панели и редакторе, бэкенд центровки без предварительного нуля (захват в MPos,
  `G10 L2` из решения), карточка «Рабочий ноль» + экран выбора метода, мастер
  «по точкам, вручную» со снятием флага `fiducialRegistration`
  (✅ 2026-07-18, PR #709/#710/#712/#713/#714). Метод «по отверстиям щупом» —
  после железной проверки 3D-touch-probe (#675).
...
*Ревизия 2026-07-18. Этот файл — нарратив фаз; конкретные задачи и статусы — на
