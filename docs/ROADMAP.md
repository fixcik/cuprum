...
- [x] **Шелк «бусинами» на сильном зуме** (✅ 2026-06-03). Оказалось — не артефакт
  ...
  (`preview.rs`).
- **(опц.) Склейка штрихов шелка в один полилайн.** `svg.rs` эмитит каждый сегмент
  глифа отдельным `<path … stroke-linecap="round">`. Сшивание связанных сегментов
  одной ширины в единый `<path>` (с `stroke-linejoin="round"`) уменьшит объём SVG
  и число DOM-нод и сгладит экстремальный зум в корректных движках. Не баг —
  улучшение; правка вывода требует бампа тега `svg-vN` (см. `DEVELOPMENT.md`).
