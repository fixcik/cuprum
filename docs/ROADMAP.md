...
- **Не рендерить SVG-слой по нескольку раз.** Композитное превью дизайна строится
  ...
  render-once).
- **Шелк «бусинами» на сильном зуме.** На большом увеличении (≈40×, текст шелка
  во весь экран) глифы рисуются не сплошными, а из отдельных штрихов с круглыми
  концами: `svg.rs` эмитит каждый примитив штриха отдельным `<path … fill="none"
  stroke-linecap="round">`, и круглые концы выпирают на каждом стыке — видны как
  бусины/швы между сегментами. На обычном зуме сливается, заметно только вблизи.
  Эмиссия не менялась с начального коммита (т.е. давнишний артефакт рендера, не
  свежий регресс). Идея: сшивать штрихи одного глифа в одну ломаную (`stroke-linejoin
  round`, единый `<path>`) либо рисовать шелк заливкой контуров; правка вывода
  требует бампа тега `svg-vN` (см. `DEVELOPMENT.md`).
