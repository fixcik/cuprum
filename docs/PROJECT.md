...
> **Сейчас vs цель.** Фактический `PanelDoc` пока хранит только размер и
> panel-space origin (FR4-бланк). `BoardInstance[]` / `ToolingHole[]` /
> `Fiducial[]` — целевая схема (см. черновик ниже и роадмап Phase 1 в
> [`ROADMAP.md`](ROADMAP.md)), ещё не реализованы.
...
│   ├── instances[]          ← BoardInstance (цель)
│   └── tooling_holes[]      ← ToolingHole (цель)
│
...
### Черновик схемы Panel (целевой, поле `manifest.panel`)

> Сейчас `PanelDoc` хранит только `schema_version`, `width_mm`, `height_mm`,
> `origin_x_mm`, `origin_y_mm`. Ниже — **целевая** схема с `instances[]` и
> `tooling_holes[]`.
...
| Сейчас | Целевое |
|---|---|
| `manifest.placements[]` (per-gerber) | Удалить; заменить `panel.instances[]` (BoardInstance) с `layer_ref` |
...
*Ревизия 2026-06-01. Обновлять по мере фиксации новых решений.*
