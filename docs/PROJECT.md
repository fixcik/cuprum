...
### Calibration через multi-layer .goo (без физического Stouffer wedge)
...
Помимо «пирамиды экспозиции», Cuprum может генерить и другие
калибровочные шаблоны — все они описываются как special `.goo` без
boardinstance:

- **минимальная ширина дорожки** — параллельные линии 50/100/150/200/
  250 µm; смотришь, какие выжили после проявки + травления;
- **минимальный зазор** — пары линий с разными промежутками;
- **анизотропия пикселей** — круги и линии под 45°; пиксели 14×19 µm
  дают асимметрию;
- **маска dose-test** — то же что «пирамида», но с более длинной
...
├── pattern_kind             ← exposure_pyramid | min_trace | min_space | …
├── pattern_params           ← N шагов, размер квадрата, …
├── time_per_step_s          ← для пирамиды
├── result.held_step         ← главный результат (для пирамиды)
├── result.min_feature_um    ← для шаблона ширины/зазора
├── screen_hours_used
└── notes
```

Cuprum сам **генерит шаблон**, считает рекомендованную дозу из
результата, и сохраняет запись. Это и есть калибровочный инструмент: «у
меня новая плёнка / новый травитель / я полгода не печатал — прогоню
пирамиду, получу новое дефолтное время».
...
**Авто-расстановка по дефолту.** При создании контурного шага Cuprum:
...
**Drag для коррекции.** UI редактора контура позволяет:

- перетянуть таб мышкой вдоль периметра,
- удалить таб (клик + Del),
- добавить таб щелчком по контуру,
- посмотреть в 3D-превью, где плата держится.

**G-code эмиссия.** На участках tab фреза поднимается до
