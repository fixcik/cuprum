...
**ProcessStep[]** (что с ней делать). Размер Panel задан в `panel.json`;
внутри лежат `BoardInstance[]`, `ToolingHole[]` и опциональные `Fiducial[]`.
...
| **Panel** | Композиция BoardInstance'ов на FR4 | `panel.json` |
...
│
├── Panel (panel.json)       ← физическая заготовка FR4
│   ├── размер и origin
...
|---|---|---|
| **ToolingHole** | `panel.json` | Механика: штифты, переворот, перенос между станками |
| **Panel-level Fiducial** | `panel.json` (опционально, рядом с ToolingHole) | Оптика на уровне всей Panel (для CNC точного нуля по камере) |
...
| `manifest.imports[]` | Переименовать в `manifest.designs[]` (сущность = Design) |
