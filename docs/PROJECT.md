...
Прямоугольная область на Panel, запрещающая размещение:
...
| `kind` | `fixture` \| `dead` \| `reserved` |

Правила по типу:

| Тип | Платы | Тех. отверстия |
|---|---|---|
| `fixture` (дефолт) | ❌ | ✅ |
| `dead` | ❌ | ❌ |
| `reserved` | ❌ | ✅ |
...
обходит зоны всех типов, а панельный DFM-вердикт (`evaluatePanel`) даёт block-находку
при пересечении платы с любой зоной (`keep-out`) и tooling-отверстия с `dead`-зоной
(`keep-out-tooling`). Зоны редактируются полностью: рисование, выделение, перемещение,
смена типа, удаление и **ресайз за 4 угловых хэндла** (снап 1 мм, кламп в панель, мин. размер).
...
*Ревизия 2026-06-05. Обновлять по мере фиксации новых решений.*
