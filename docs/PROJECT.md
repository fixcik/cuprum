...
- **По реперам** (центровка) — для последующих операций над уже размеченной
  платой. Оператор наводит шпиндель в реальные реперные отверстия (jog) и
  фиксирует их рабочие (G54) позиции; по парам (идеальная work(G54)-координата репера →
  измеренная) фитится **2D-similarity-трансформа** (сдвиг + поворот + равномерный
  масштаб) с RMS-невязкой. Трансформа **транзиентна** (на сессию станка, не в
  `.cuprum`) и применяется к координатам отверстий при эмиссии G-code. GRBL не
  умеет поворот СК (нет G68), поэтому поворот/масштаб считаются и применяются
  софтово.
...
*Ревизия 2026-07-17. Обновлять по мере фиксации новых решений. Последнее изменение: точки центровки (`AlignmentPoint`) в модели панели, схема PanelDoc v5.*
