...
> **Сейчас vs цель.** `PanelDoc` (схема v3) несёт `instances[]`
> (`BoardInstance`), `tooling_holes[]` (`ToolingHole`) и `keep_out_zones[]`
> (`KeepOutZone`) как структуры данных. `Fiducial[]` пока не реализован.
...
- **По реперам** (центровка) — для последующих операций над уже размеченной
  ...
  фиксирует их машинные позиции; по парам (идеальная machine-координата репера →
...
*Ревизия 2026-06-15. Обновлять по мере фиксации новых решений. Последнее изменение: авторасстановка реперов (`fiducial_params`) и центровка CNC-операций по реперам (2D-similarity-трансформа, два режима базирования).*
