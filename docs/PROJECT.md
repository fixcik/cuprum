...
`panel`, схема v5 — раньше был отдельный `panel.json`); внутри лежат
`BoardInstance[]`, `ToolingHole[]`, `KeepOutZone[]` и опциональные `Fiducial[]`.

> **Сейчас vs цель.** `PanelDoc` (схема v5) несёт `instances[]`
> (`BoardInstance`), `tooling_holes[]` (`ToolingHole`), `keep_out_zones[]`
> (`KeepOutZone`) и `alignment_points[]` (`AlignmentPoint`) как структуры
> данных. `Fiducial[]` пока не реализован.

### Панель — машинно-независимая сущность

`PanelDoc` задаёт **размер заготовки** и **области раскладки** плат. Ограничений
конкретного станка она не несёт: `width_mm`/`height_mm` — физические размеры FR4,
выбранного пользователем, а не рабочая зона принтера или CNC.

Проверка «панель (или отдельный проход раскладки) влезает в рабочую зону выбранного
станка» — операционная, выполняется **на этапе ProcessStep** под конкретный
`MachineProfile`. Такая декомпозиция позволяет редактировать раскладку и считать
DFM без подключённого станка и независимо от того, на каком оборудовании будет
печататься данная панель. Работа по интеграции этой проверки ведётся в рамках
эпика #178; follow-up задача — #246.
...
│   ├── keep_out_zones[]     ← KeepOutZone (реализовано)
│   └── alignment_points[]   ← AlignmentPoint (реализовано)
│
...
> Размещение **side-agnostic**: одна плата занимает свой слот на Panel сразу с
> обеих сторон, поэтому у `BoardInstance` нет привязки к стороне. Верх/низ — это
> забота операции (засветка/сверловка), а не раскладки. (Раньше было поле
> `layer_ref: Top|Bottom`; удалено — старые `.cuprum` грузятся, поле игнорируется.)
...
**Производная зона зажима.** Отверстия ролей `registration`/`flip` несут вокруг себя
keep-out зону зажима (квадрат: сторона = `diameter_mm + 2·toolingClampRadiusMm`, по центру
отверстия), куда нельзя класть платы — палец/прижим физически занимает место. Зона
**производная**, не хранится в проекте: вычисляется из отверстий + поля `toolingClampRadiusMm`
профиля станка (по умолчанию 0 = выключено), поэтому следует за отверстием и исчезает вместе
с ним. Семантика `fixture` (платы нельзя, tooling можно). Влияет на авто-раскладку (обходится)
и панельный DFM-вердикт (block-находка при пересечении). Роль `unused` зону не несёт.
...
**Авторасстановка реперов.** `PanelDoc.fiducial_params` (опционально, persist в
`.cuprum`) задаёт параметры авто-генерации реперных `ToolingHole`'ов ролью
`registration`: ось ряда (X/Y), количество, шаг, диаметр и смещение от центра.
Реперы раскладываются **симметрично относительно центра** панели по выбранной
оси (привязка к центру, не к краю — чтобы точность совмещения не зависела от
точности замера габарита заготовки). Поле опциональное: старые проекты без него
читаются (serde default).

### AlignmentPoint (точка центровки)

Явная точка центровки на панели — пользователь ставит её там, где будет
захватывать реальное положение заготовки при привязке рабочего нуля
(ручная наводка по 2+ точкам или промер щупом — применение «(планируется)»):

| Поле | Смысл |
|---|---|
| `id` | Стабильный id точки (`ap-1`, …) |
| `x_mm`, `y_mm` | Положение в Panel space (мм) |
| `hole_diameter_mm` | Диаметр физического отверстия, к которому точка привязана; `None` = свободная точка (только визуальный захват, щупом не промеряется) |

Хранится в `PanelDoc.alignment_points[]` (схема v5; старые проекты грузятся с
пустым списком). **Щупабельность** — производный признак
(`AlignmentPoint::is_probeable`): точка промеряется щупом, только если привязана
к отверстию диаметром ≥ `PROBEABLE_MIN_HOLE_DIAMETER_MM` (2.0 мм, предварительно
— уточняется на железе, задача #675).

**Реперные отверстия не дублируются.** Каждый `ToolingHole` с ролью
`registration` сам выступает точкой центровки, привязанной к собственному
отверстию; `PanelDoc::effective_alignment_points()` возвращает их (с id
отверстия и его диаметром) плюс явные точки — явных копий в
`alignment_points[]` не хранится.

### Центровка по реперам (CNC)

Базирование CNC-операций (сверловка/раут) к реальному положению платы на столе —
**два режима**:

- **По углу** (`DatumCorner`) — work-zero выбором угла панели; для *изначальной*
  сверловки самих реперов в голой заготовке (реперов ещё нет).
- **По реперам** (центровка) — для последующих операций над уже размеченной
  платой. Заранее выставленный рабочий ноль **не нужен**: оператор наводит
  шпиндель в реальные реперные отверстия (jog) и фиксирует их **машинные**
  (MPos) позиции; по парам (идеальная панельная/work-координата репера →
  измеренная машинная) фитится **2D-similarity-трансформа** (сдвиг + поворот +
  равномерный масштаб) с RMS-невязкой. Сдвиг трансформы программно становится
  нулём G54 (`G10 L2 P1`, без движения шпинделя, с ожиданием `ok`), а остаток
  (поворот/масштаб, нулевой сдвиг) применяется к координатам отверстий при
  эмиссии G-code. Остаточная трансформа **транзиентна** (на сессию станка, не в
  `.cuprum`). GRBL не умеет поворот СК (нет G68), поэтому поворот/масштаб
  считаются и применяются софтово. Z-ноль это не трогает — он по-прежнему
  привязывается щупом на каждый инструмент.

Реперные отверстия — **единый якорь**: на UV-приспособе в них входят штифты
(совмещение верх/низ), на CNC они свободны (плата в тисках) и служат точками
центровки. Захват реальной позиции — ручной (наводка глазом/конусом/камерой,
способ не зашит); авто-центрирование отверстия щупом — отдельная
задача-исследование. Габарит заготовки в точность совмещения не входит — её
несут реперы.

### KeepOutZone

Прямоугольная запрещённая область на Panel. Единый тип: ни платы, ни
тех. отверстия не должны заходить в зону; станок обходит её при сверловке.

| Поле | Смысл |
|---|---|
| `id` | Стабильный id зоны (`koz-…`, UUID) |
| `x_mm`, `y_mm` | Левый верхний угол в Panel space (мм) |
| `width_mm`, `height_mm` | Размер (всегда положительный) |

Зоны влияют на раскладку и вердикт (Фаза 2): авто-раскладка (`panelObstacles`)
обходит все зоны, а панельный DFM-вердикт (`evaluatePanel`) даёт единую
block-находку `keep-out` при пересечении зоны платой или тех. отверстием.
Зоны редактируются полностью: рисование, выделение, перемещение,
удаление и **ресайз за 4 угловых хэндла** (снап 1 мм, кламп в панель, мин. размер).

Схема `PanelDoc` при открытии старого документа без поля `keep_out_zones`
автоматически подставляет пустой список (`#[serde(default)]`).
...
свёрл. Плюс **3D-превью**: `substrate_thickness_mm` задаёт высоту подложки
(и Z всех слоёв/стенок/бочек) в меше — `mesh::board_geometry` строится по этой
толщине; когда Stackup ещё не настроен, берётся дефолт `DEFAULT_FR4_THICK` (1.6).
...
- **Machine space** — проекция панели на конкретный процесс:
  - UV: `compose_layout` масштабирует/сдвигает panel → экран 15120×6230,
    учитывает mirror/invert/rotate принтера. **При обработке нижней стороны**
    (выбор стороны — параметр операции, не размещения) делается **mirror по
    оси Y** (X-координата зеркалится), чтобы при перевороте заготовки на
    штифтах нижняя медь легла правильно.
...
### Каталог недавних (`catalog.sqlite`)

Машинно-локальная SQLite-база в `app_data_dir` ОС — MRU-список для домашнего
экрана. Источник истины — `.cuprum`-файл; каталог хранит только кешированные
свойства для карточки без чтения контейнера:

| Столбец | Тип | Смысл |
|---|---|---|
| `path` | TEXT UNIQUE | Абсолютный путь к `.cuprum` |
| `name` | TEXT | Название проекта |
| `last_opened_at` | INTEGER | Unix-timestamp последнего открытия |
| `design_count` | INTEGER | Число дизайнов (footprint карточки) |
| `width_mm`, `height_mm` | REAL | Размер заготовки панели (null до настройки) |
| `panel_verdict` | TEXT | Кешированный вердикт панели (`ok`/`warn`/`block`); null до первого расчёта |
| `profile_hash` | TEXT | Хеш профиля станка, при котором был вычислен `panel_verdict`; null до первого расчёта |

Вердикт **не хранится в `.cuprum`** — это свойство связки «проект × станок»,
а не самого проекта. Карточка домашнего экрана показывает вердикт только когда
`profile_hash` совпадает с текущим профилем; иначе — нейтральная точка (нужно
переоткрыть проект). Хеш (`djb2` по сортированному JSON всех полей
`CapabilityProfile`) вычисляется клиентом (`lib/profileHash.ts`).

Схема создаётся через `CREATE TABLE IF NOT EXISTS`, новые столбцы добавляются
`ALTER TABLE ... ADD COLUMN` с проглатыванием ошибки «duplicate column name» —
тот же паттерн, что и для `design_count`/`width_mm`.
...
> размера дизайна — против **размера Panel**; размер Panel не клампится по
> рабочей зоне станка (панель машинно-независима — см. выше).
...
| Сейчас | Целевое |
|---|---|
| `manifest.placements[]` (per-gerber) | ✅ Удалено (манифест → схема v5); размещение переехало в `panel.instances[]` (`BoardInstance`) |
...
| `placement.side` | ✅ Размещение side-agnostic (плата на обе стороны); сторона — параметр операции засветки/сверловки, не `BoardInstance` |
...
2. **Двусторонка — одна панель**, размещение одно (плата на обе стороны);
   сторона обработки выбирается операцией, переворот заготовки через pins.
...
## Межоконная передача позиции станка

`machine://status` — глобальное Tauri-событие с полями `{state, mpos, wpos}`,
транслируемое GRBL-поллером в реальном времени. Используется для передачи живой
позиции шпинделя в webview окна сверловки (отдельный процесс), минуя Channel-
телеметрию главного окна. Хук `useMachinePosition` подписывается на это событие и
выставляет актуальные рабочие координаты (`wpos[0]`, `wpos[1]`) в мм; позиция
обнуляется через 1 с молчания (поллер остановился / кабель отключён).
...
*Ревизия 2026-07-18. Обновлять по мере фиксации новых решений. Последнее изменение: центровка по реперам без предварительного рабочего нуля — захват в машинных координатах (MPos), ноль G54 программируется из решения (`G10 L2`), к отверстиям применяется только остаточный поворот/масштаб.*
