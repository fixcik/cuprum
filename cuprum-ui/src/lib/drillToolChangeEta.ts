 ...
 *  range. Uses the Rust per-group motion buckets (`groupMotionSecs`), scaling the
 *  current group's bucket by the fraction of its holes still undrilled (linear within
 *  the group — per-group time is exact, intra-group is proportional).
 *
...
export function toolChangeEta(
  ...
  for (let gi = 0; gi < groups.length; gi++) {
    ...
    if (n > 0 && holesCompleted < acc + n) {
      ...
      const groupSec = groupMotionSecs[gi] ?? 0;
