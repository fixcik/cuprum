...
use crate::viewer::geometry::mirroring::Mirroring;
...
impl GerberTransform {
    ...
    pub fn to_matrix(&self) -> Matrix3<f64> {
        ...
        let scaling_matrix =
            Matrix3::new(self.scale, 0.0, 0.0, 0.0, self.scale, 0.0, 0.0, 0.0, 1.0);
...
mod transform_tests {
    ...
    fn test_combined_transforms_example() {
        ...
        println!(
            "box1_final_matrix: {:?}",
            [box1_final_matrix.x, box1_final_matrix.y]
        );

        let box2_final_matrix = combined2_matrix.apply_to_position_matrix(box2_position);
        println!(
            "box2_final_matrix: {:?}",
            [box2_final_matrix.x, box2_final_matrix.y]
        );
    ...
    fn test_rotation_around_different_centers() {
        ...
        println!(
            "Sequential for box1: {:?}",
            [box1_after_global.x, box1_after_global.y]
        );
        println!(
            "Combined for box1: {:?}",
            [box1_after_combined.x, box1_after_combined.y]
        );
        println!(
            "Sequential for box2: {:?}",
            [box2_after_global.x, box2_after_global.y]
        );
        println!(
            "Combined for box2: {:?}",
            [box2_after_combined.x, box2_after_combined.y]
        );
...
impl Matrix3TransformExt for Matrix3<f64> {
    ...
    fn is_90_or_270_rotation(&self) -> bool {
        ...
        let off_diagonals_opposite_sign =
            (b * c) < 0.0 && b.abs() > f64::EPSILON && c.abs() > f64::EPSILON;
