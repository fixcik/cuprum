...
pub fn pack(workdir: &Path, container: &Path) -> Result<()> {
    ...
    // Reclaim gerber dirs no live manifest/restore-point references (deleted designs).
    let live_dirs = live_gerber_dirs(workdir, &manifest);
...
/// any restore point's (migrated) snapshot manifest.
fn live_gerber_dirs(workdir: &Path, manifest: &Manifest) -> std::collections::HashSet<String> {
    let mut live = std::collections::HashSet::new();
    referenced_gerber_dirs(manifest, &mut live);
    if let Ok(metas) = crate::document::history::list(workdir) {
        for meta in metas {
            if let Ok(snap) = crate::document::history::read(workdir, &meta.id) {
