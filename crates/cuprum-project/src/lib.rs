...
/// Disambiguate a basename against names already used in the same import:
...
fn build_entries(
    ...
    for (idx, imp) in imports.iter().enumerate() {
        ...
        manifest_imports.push(manifest::Import {
            id,
            source_name: imp.source_name.clone(),
            gerbers,
        });
    }
    (manifest_imports, entries)
}
...
pub fn create_project(
    ...
    let (manifest_imports, entries) = build_entries(&imports);
    let mut manifest = Manifest::new(name);
    manifest.imports = manifest_imports;
...
pub fn import_zips(
    ...
    for imp in &existing.imports {
        let mut gerbers = Vec::new();
        for g in &imp.gerbers {
    ...
    for (i, old) in existing.imports.iter().enumerate() {
        if let Some(new_imp) = manifest.imports.get_mut(i) {
            for (g, old_g) in new_imp.gerbers.iter_mut().zip(old.gerbers.iter()) {
...
/// Already-existing imports keep their layer types (see `import_zips`).
pub fn commit_import(
    ...
    // Number of imports already present, so we only re-type the newly added ones.
    ...
    let new_gerbers = manifest
        ...
        .iter_mut()
        .skip(existing_count)
        .flat_map(|imp| imp.gerbers.iter_mut());
...
mod tests {
    ...
    fn create_open_import_flow() {
        ...
        assert_eq!(m.imports[0].gerbers[0].path, "gerbers/import-1/board.gbr");
        ...
        assert_eq!(opened.imports.len(), 1);
    ...
    fn commit_import_applies_layer_types_positionally() {
        ...
        assert_eq!(m.imports.len(), 1);
        assert_eq!(m.imports[0].gerbers[0].path, "gerbers/import-1/board.gbr");
        assert_eq!(m.imports[0].gerbers[0].layer_type, LayerType::TopCopper);
